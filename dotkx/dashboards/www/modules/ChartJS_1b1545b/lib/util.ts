...
class Util {
    // https://www.geeksforgeeks.org/javascript/how-to-convert-color-names-to-hexcode-using-javascript/
    public static readonly HEX_CODES = {
        aliceblue: "#f0f8ff",
        antiquewhite: "#faebd7",
        aqua: "#00ffff",
        aquamarine: "#7fffd4",
        ...
        if (_.isArray((options as LoadCache).load)) {
            const cachedAttributes = (options as LoadCache).load.reduce(
                (a, b: string, i: number): {} => {
                    ...
                    if (cached && cached.get && cached.get("_viewType")) {
        ...
        if (isFlatArray) {
            return Object.keys(object)
                ...
                .reduce((a: T, b: Fragment): T => {
                    ...
                    return _.merge(a, Util.pump(value));
                }, []);
        }
    ...
    public static borderFactory(prefix: string): DynamicSchema<PrimitiveProperty> {
        ...
        return _.merge(Util.colorFactory(prefix + "Border"), borderSettings);
    }
