...
onmessage = function(e) {
    ...
    if (e.data == "queue") {
        ...
        const filterUnique = e.data[4];
        const sorted = e.data[5];
        const sortedXCol = e.data[6];
        ...
        if (LONG_TYPES.indexOf(colType) !== -1 && isIE11) {
            ...
            if (axisType === "Linear" || axisType === "Time" || axisType === "Logarithmic") {
                ...
                if (!isBounds) {
                    indices.sort(function(x, y) {
                        const val1 = colData[x];
                        const val2 = colData[y];

                        // place nulls at end
                        if (isNaN(Number(val1)) && isNaN(Number(val2))) return 0;
                        if (isNaN(Number(val1))) return 1;
                        if (isNaN(Number(val2))) return -1;
                ...
                if (sorted) {
                    ...
                    if (!isBounds) {
                        indices.sort(function(x, y) {
                            ...
                            if (
                                !sortedXCol &&
                                (colType === 10 || colType === 11) &&
                                isNaN(Number(val1)) &&
                                isNaN(Number(val2))
                            ) {
                                return colData[x].localeCompare(colData[y]);
                            }
                    ...
                    if (filterUnique) {
                        ...
                        colData.forEach((val, i) => {
                            const indexToUse = sorted ? indices[i] : i;
                            const objVal = valuesObj.get(val);
                            if (!objVal) valuesObj.set(val, [indexToUse]);
                            else objVal.push(indexToUse);
                        });
